...
const getProductsId = async (req, res, next) => {
  try {
    ...
    if (!product) res.status(404).send("Product doesn't exists");
...
const getProductsByCategory = async (req, res, next) => {
  try {
    ...
    if (!category) res.status(400).send("Not valid query!");
    const products = await Products.findAll({
      include: {
        model: Categories,
        where: {
...
const getProductsBySearch = async (req, res, next) => {
  try {
    const { key } = req.query;
    if (!key) res.status(400).send("Not valid query!");
    const products = await Products.findAll({
      where: {
        name: {
          [Op.or]: {
...
const postProduct = async (req, res, next) => {
  try {
    if (!Array.isArray(req.body) && typeof req.body === "object") {
      ...
      if (!wasCreated) return res.status(302).send("Product already exists.");
      ...
      const categoriesArrayOfArrays = req.body.map((product) =>
        product.categories.split(" ").map((category) => ({ name: category }))
      );
      const builtProducts = [];
      ...
      if (wasIndeedBuild.includes(false))
        return res.status(302).send({
          ...
          ...products[wasIndeedBuild.indexOf(false)],
        });
    ...
    if (error.message === "Validation error") {
      const errors = error.errors.map((errorInstance) => ({
        ERROR: "Validation error!!",
        message: errorInstance.message,
        instance: errorInstance.instance,
      }));
...
const putProduct = async (req, res, next) => {
  try {
    ...
    if (!updated) res.status(404).send("Product not found");
    res.status(200).send(product);
  } catch (error) {
    next(error);
  }
